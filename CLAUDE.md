...
| 9 | 메타 업그레이드 가격 (actualCost 계산 등) | ⚠️ 부분 완료 |
| 12 | 플레이어 HP 40→70 | ✅ |
| 13 | 연쇄고리 val:3→2 | ✅ |
...
### 세션 1: 밸런스 완성
- **#9** 메타 업그레이드 가격 표시: 구매 버튼 `⭐{u.cost}` → `⭐{actualCost}` 교체
- **#11** 몬스터 HP x1.3 (20종 개별 교체)
- **#10** 전투화면 overflow 고정 (`height:"100vh", minHeight:"auto", overflow:"hidden"`)
