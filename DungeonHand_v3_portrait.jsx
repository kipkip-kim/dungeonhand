...
const RELICS = [
  ...
  { id: "chain", name: "연쇄의 고리", emoji: "⛓️", desc: "스트레이트 배율 +3", tier: 2, eff: { type: "handAdd", hand: "스트레이트", val: 3 } },
  ...
  { id: "hero", name: "영웅의 증표", emoji: "🏅", desc: "스트레이트 플러시 배율 x3", tier: 3, eff: { type: "handMul", hand: "스트레이트 플러시", val: 3 } },
...
function calcDamage(cards, hand, relics, passiveState) {
  ...
  // Common card: focus
  cards.forEach(function(c) {
    if (c.isCommon && c.common.fx === "focus") mult += 1.0;
  });
  // Keyword: resonance
  cards.forEach(function(c) {
...
function CardView(props) {
  ...
  var fxText = "";
  if (isC) {
    var fxMap = { fortress: "방어막+" + (c.grade + (c.growthBonus || 0)), aimed: "다음턴 제출+1", wild: "조건부 와일드", focus: "배율+1.0" };
  ...
  return (
    ...
    >
      ...
      <span style={{
        ...
        {c.grade + (c.growthBonus || 0)}{c.enhanced ? "⬆" : ""}
      </span>
      <span style={{
        fontSize: small ? 8 : 10,
...
export default function DungeonHand() {
  ...
  var MAX_HP = 40 + upgradeLevels.hp * 5;
  ...
  function beginBattle(curDeck, curRelics, fl, bn) {
    ...
    var mhp = Math.floor(m.hp * (1 + (fl - 1) * 0.15));
        ...
        if (c.keyword && c.keyword.id === "growth") {
          setDeck(function(d) {
            return d.map(function(dc) {
              return dc.id === c.id ? Object.assign({}, dc, { growthBonus: (dc.growthBonus || 0) + 1 }) : dc;
            });
          });
        }
      });
  ...
  function enhanceCard(card) {
    var newDeck = deck.map(function(c) {
      return c.id === card.id ? Object.assign({}, c, { grade: c.grade + 1, enhanced: true }) : c;
    });
    setDeck(newDeck);
    advanceBattle(newDeck);
  }
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {UPGRADES.filter(function(u) { return u.tier === "basic"; }).map(function(u) {
                ...
                var maxed = lv >= u.max;
                var canBuy = metaPoints >= u.cost && !maxed;
                return (
                  <div key={u.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: maxed ? "#22c55e11" : "var(--cd)", border: "1px solid " + (maxed ? "#22c55e44" : "var(--bd)"), borderRadius: 8, padding: "8px 12px" }}>
                    ...
                    {maxed ? (
                      ...
                      <Btn
                        onClick={function() {
                          if (!canBuy) return;
                          setMetaPoints(function(p) { return p - u.cost; });
  ...
  if (screen === "campfire") {
    ...
    function resolveCampfire() {
      ...
      if (evtId === "ambush") {
        ...
        var amhp = Math.floor(am.hp * (1 + (floor - 1) * 0.15));
  ...
  if (screen === "enhance") {
    var enhanceable = deck.filter(function(c) { return !c.enhanced; });
    var suitOrder = { red: 0, blue: 1, yellow: 2 };
    ...
    return (
      <div style={wrapStyle}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, padding: 16, overflow: "auto" }}>
          <h3 style={{ fontSize: 14 }}>강화할 카드 (등급+1)</h3>
